import{ArrowRightIcon,CheckCircle2Icon,XCircleIcon,LoaderCircleIcon}from"lucide-react"
...
export function VerificationPanel({comparison}:{comparison:VerificationComparison}){
 ...
 return <Card className="verification-card"><CardHeader><CardTitle>{pending?<LoaderCircleIcon className="animate-spin"/>:comparison.passed?<CheckCircle2Icon/>:<XCircleIcon/>}{pending?"Verifying recovery…":comparison.passed?"Simulator verified recovery":"Verification failed"}</CardTitle></CardHeader><CardContent>{rows.map(([label,before,after])=><div className="comparison-row" key={label}><span>{label}</span><strong>{before}</strong><ArrowRightIcon/><strong>{after}</strong></div>)}</CardContent></Card>
}

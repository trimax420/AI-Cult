...
const stateCopy = {
  ...
  PRODUCTION_SAVED: { title: "Production Saved — Trailer delivery protected.", detail: "Simulator verification confirms throughput recovery and an on-time ETA." },
}
...
function App() {
  ...
  const overview = !operations.diagnosis || !state ? <div className="loading-grid">{Array.from({ length: 8 }, (_, index) => <Skeleton key={index} />)}</div> : <div className="operations-grid">
    <section className="primary-column">
      ...
      {operations.briefing ? <CopilotPanel key={`copilot-${state.incident_started_at ?? "healthy"}`} briefing={operations.briefing} ask={operations.askCopilot} onApprove={approve} busy={operations.busy} /> : null}
    </section>
    <RecoveryPanel plans={operations.plans} isApproving={operations.busy || (!!state.recovery_progress && !state.verification_complete && !state.recovery_failed)} onApprove={approve} />
  ...
  return <TooltipProvider><SidebarProvider defaultOpen><AppSidebar activeView={view} incidentCount={incidentCount} onNavigate={setView} /><SidebarInset>
    ...
    <main className="dashboard-shell">
      ...
      {operations.agentError ? <p role="status" className="connection-error">Agent unavailable: {operations.agentError}. Deterministic simulator options remain available.</p> : null}
      {operations.error ? <div className="connection-error">Unable to load live data: {operations.error}</div> : null}
      {view === "overview" ? overview : !operations.diagnosis || !state ? <div className="loading-grid"><Skeleton /><Skeleton /><Skeleton /></div> : <OperationsTabs view={view} state={state} diagnosis={operations.diagnosis} audit={operations.audit} plans={operations.plans} isApproving={operations.busy || (!!state.recovery_progress && !state.verification_complete && !state.recovery_failed)} onApprove={approve} />}
